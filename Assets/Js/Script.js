...
(function(){
  ...
  // Service Request Forms (Recharge & Ticket Booking Pages)
  var serviceForm=document.getElementById('serviceForm');
  if(serviceForm){
    serviceForm.addEventListener('submit',function(e){
      e.preventDefault();
      var status=document.getElementById('serviceFormStatus');
      var btn=serviceForm.querySelector('button[type="submit"]');
      var scriptUrl=window.APPS_SCRIPT_URL||'';
      if(!scriptUrl||scriptUrl.indexOf('YOUR_')!==-1){
        if(status){status.textContent='Please Configure Google Apps Script URL First.';status.className='form-status error';}
        return;
      }
      btn.disabled=true;
      ...
      var serviceType=serviceForm.getAttribute('data-service')||'Service Request';
      var fields=serviceForm.querySelectorAll('[name]');
      var details=[];
      var name='', email='', phone='';
      fields.forEach(function(el){
        var val=(el.value||'').trim();
        if(!val) return;
        var label=el.getAttribute('data-label')||el.getAttribute('placeholder')||el.name;
        if(el.name==='name') name=val;
        else if(el.name==='email') email=val;
        else if(el.name==='phone') phone=val;
        else details.push(label+': '+val);
      });
      var message='[Service: '+serviceType+']\n'+details.join('\n');
      if(serviceForm.querySelector('[name="notes"]')){
        var notes=(serviceForm.querySelector('[name="notes"]').value||'').trim();
        if(notes) message+='\nNotes: '+notes;
      }
      ...
      fetch(scriptUrl,{
        ...
        if(status){status.textContent='Thank You! Your Request Has Been Received. We Will Contact You Shortly.';status.className='form-status success';}
        serviceForm.reset();
      }).catch(function(){
        if(status){status.textContent='Something Went Wrong. Please Try WhatsApp.';status.className='form-status error';}
      }).finally(function(){
