...
// All form data sheets (auto-created)
var FORM_SHEETS = [
  'Index',
  'Vignesh Digital Hub',
  'Vignesh Digital Electronics Service',
  'Vignesh Pixel Works',
  'Vignesh Digital Marketing',
...
var FORM_HEADERS = ['Timestamp', 'Name', 'Email', 'Phone', 'Business', 'Message', 'Source'];
...
function ensureSheets(ss) {
  // Create all form / business / service sheets
  for (var i = 0; i < FORM_SHEETS.length; i++) {
    ensureFormSheet(ss, FORM_SHEETS[i]);
  }

  // Keep legacy Contacts sheet if it exists (do not delete user data)
  var legacy = ss.getSheetByName('Contacts');
  if (legacy) {
    var headers = legacy.getRange(1, 1, 1, Math.max(legacy.getLastColumn(), 1)).getValues()[0];
    if (headers.indexOf('Phone') === -1 && legacy.getLastColumn() >= 3) {
      legacy.insertColumnAfter(3);
      legacy.getRange(1, 4).setValue('Phone');
    }
  }
...
function ensureFormSheet(ss, name) {
  var sheet = ss.getSheetByName(name);
  if (!sheet) {
    sheet = ss.insertSheet(name);
    sheet.appendRow(FORM_HEADERS);
    return sheet;
  }
  // Ensure header row has expected columns
  var lastCol = Math.max(sheet.getLastColumn(), 1);
  var headers = sheet.getRange(1, 1, 1, lastCol).getValues()[0];
  if (!headers[0] || String(headers[0]).toLowerCase().indexOf('timestamp') === -1) {
    sheet.insertRowBefore(1);
    sheet.getRange(1, 1, 1, FORM_HEADERS.length).setValues([FORM_HEADERS]);
  } else if (headers.indexOf('Source') === -1) {
    sheet.getRange(1, headers.length + 1).setValue('Source');
  }
  return sheet;
}

function appendFormRow(ss, sheetName, data) {
  var sheet = ensureFormSheet(ss, sheetName);
  sheet.appendRow([
    new Date(),
    data.name || '',
    data.email || '',
...
function saveContact(data) {
  ...
  // Priority: explicit sheet/source → service form → Index (home/contact forms)
  var source = String(data.source || data.sheet || '').trim();
  ...
  var serviceMap = {
    ...
    'Utility Bill Payments': 'Utility Bill Payment',
    'Bus Ticket Booking': 'Bus Ticket Booking',
  ...
  if (serviceMap[source]) source = serviceMap[source];

  var business = data.business || '';
  var savedSheets = [];

  // 1) Always save to primary source sheet (Index or service sheet)
  appendFormRow(ss, source, {
    name: data.name,
    email: data.email,
    phone: data.phone,
    business: business,
  ...
  savedSheets.push(source);

  // 2) If Index form with a business selected → also save to that business sheet
  if (source === 'Index' && business && BUSINESS_EMAILS[business]) {
    if (savedSheets.indexOf(business) === -1) {
      appendFormRow(ss, business, {
        name: data.name,
        email: data.email,
        phone: data.phone,
        business: business,
      ...
      savedSheets.push(business);
    }
  }

  // 3) Service requests also copy into Vignesh Digital Hub business sheet
  var serviceSheetNames = [
    'Mobile Recharge', 'DTH Recharge', 'FasTag Recharge', 'Utility Bill Payment',
    'Bus Ticket Booking', 'Train Ticket Booking', 'Flight Ticket Booking', 'Movie Ticket Booking'
  ];
  var isService = serviceSheetNames.indexOf(source) !== -1;
  if (isService) {
    var hub = 'Vignesh Digital Hub';
    if (savedSheets.indexOf(hub) === -1) {
      appendFormRow(ss, hub, {
        name: data.name,
        email: data.email,
        phone: data.phone,
        business: hub,
      ...
      savedSheets.push(hub);
    }
  }

  // Email routing
  var toEmail = BUSINESS_EMAILS[business] || FOUNDER_EMAIL;
  if (isService) {
    toEmail = BUSINESS_EMAILS['Vignesh Digital Hub'] || FOUNDER_EMAIL;
  }

  var subject = 'New Enquiry – ' + source + (business ? ' – ' + business : '') + ' – ' + (data.name || 'Visitor');
  var body =
    'New Form Submission\n\n' +
    'Source Sheet: ' + source + '\n' +
    'Name: ' + (data.name || '') + '\n' +
    ...
    'Business: ' + (business || '') + '\n' +
    'Message:\n' + (data.message || '') + '\n\n' +
    '---\nSaved In: VigneshUniverse-Data → ' + savedSheets.join(', ');
